...
function drawSelect(obj, items, options){
	...
	case 'companycode':
		...
		$.each(items, function (i, item) {
			if( item.CD_TP !=='41_tmc'
				&& item.CD_TP !=='41_ast'
				&& item.CD_TP !=='41_spfc'){
				obj.append($('<option>', { 
			        value: item.COMPANY_CODE,
			        text : item.COMPANY_CODE+'('+item.CD_TP_MEANING+')' 
			    }));
			}		
	...
	case 'tns_db':
		...
		$.each(items, function (i, item) {
			var schema = item['feed.db.schema'];
			if(schema===undefined) schema = '';
			else schema = '[schema:'+schema+']';
			obj.append($('<option>', { 
				value: item.id,		
		        text : item.id+' ('+ item.desc+', '+item.type +')'+schema,
		        tag : item['feed.db.schema']
		    }));
		});
		break;
	case 'tns_idx':
		...
		var select_host= $('#sel_host option:selected').text().split(' (')[0];
		var select_type= $('#sel_host option:selected').text().split(',')[1];
		//console.log("tns_idx(select_host)="+select_host);
		//console.log("tns_idx(select_type)="+select_type);
		$.each(items, function (i, item) {			
			if(item.id===select_host && select_type.indexOf(item.type)>-1){
				
				//console.log("tns_idx(item.id)="+item.id);
				//console.log("tns_idx(item.type)="+item.type);
				//console.log("tns_idx(item)="+JSON.stringify(item));
				
				var arr_sysid = item['feed.sub.sysid'].split(',');
				arr_sysid.sort();
				var url = item['feed.db.url'].split('@')[1];
				if(url==undefined)
					url = item['feed.db.url'].split('//')[1];
				url = url.length<40 
						...
						: url.split('HOST=')[1].split(')')[0] 
							+ ':' + url.split('PORT=')[1].split(')')[0]
							+ ':' + url.split('SERVICE_NAME=')[1].split(')')[0];
				$.each(arr_sysid, function (j, _sysinfo) {
					//console.log("tns_idx(_sysinfo)="+_sysinfo);
					var _sys_id = _sysinfo.split('|')[0];
					var _sys_name = _sysinfo.split('|').length>1?_sysinfo.split('|')[1]:"-";
					obj.append($('<option>', { 
						value: item['feed.db.driver']+','+item['feed.db.url']+','+item['feed.db.user']+','+item['feed.db.pwd'],		
				        text : _sys_id +' ('+ _sys_name+', '+url +', '+item['feed.db.user']+')'
				    }));
				});
			}
		});
	...
	case 'tns_idx_upsert':
		...
		//console.log("tns_idx_upsert(items)="+JSON.stringify(items));
				
		var arr_sysid = items['feed.sub.sysid'].split(',');
		arr_sysid.sort();
		var url = items['feed.db.url'].split('@')[1];
		if(url==undefined)
			url = items['feed.db.url'].split('//')[1];
		url = url.length<40 
				? url 
				: url.split('HOST=')[1].split(')')[0] 
					+ ':' + url.split('PORT=')[1].split(')')[0]
		...
		$.each(arr_sysid, function (j, _sysinfo) {
			...
			var _sys_name = _sysinfo.split('|').length>1?_sysinfo.split('|')[1]:"-";
			obj.append($('<option>', { 
				value: items['feed.db.driver']+','+items['feed.db.url']+','+items['feed.db.user']+','+items['feed.db.pwd'],		
		        text : _sys_id +' ('+ _sys_name+', '+url +', '+items['feed.db.user']+',['+ items['id']+'])'
		    }));
		});
		break;
	case 'ela_ip':
...
//http url을 호출하여 가동계 xml데이타를 조회한다. (1단계 systemID)
function getConfigDataAsJson(objnm, desc){
	var params = {};
	 ...
	 type : 'POST',
	         ...
	         success:function(data){
	        	 var json = data.result.feed.system;
					//console.log(objnm+" desc="+desc);
	        	 if(objnm==='sel_host'){
		        	 var obj = $('#sel_host');
 					// console.log(objnm+" json="+JSON.stringify(json))
		        	 drawSelect(obj, json, 'tns_db');	        		 
	        	 }
	        	 else if(objnm==='sel_host_idx'){
		        	 var obj = $('#sel_host_idx')
		        	 if(desc !== undefined){
		        		 $.each(json, function (i, item) {
		        			 if(returnbool) return false; //break
		        			 //console.log("i="+i+") "+item['feed.sub.sysid']);
			        		 var arr_sysid = item['feed.sub.sysid'].split(',');
			        		 $.each(arr_sysid, function (j, _sysinfo) {
			        			 if(returnbool) return false; //break
			        			 if(_sysinfo.split('|')[0]===desc){
			        				//console.log(objnm+"("+ desc+") item="+JSON.stringify(item));
			   		        		drawSelect(obj, item, 'tns_idx_upsert');
			   		        		returnbool = true;
			        			}
			        		 });
